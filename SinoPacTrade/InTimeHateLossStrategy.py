...
class InTimeHateLossStrategy(Strategy):
    def __init__(self, api, code, subcode, positionAction, positions, maxPositions, debugMode=True):
        super(InTimeHateLossStrategy, self).__init__(api, code, subcode, positionAction, positions, maxPositions, debugMode)
        Util.log(f"Create InTimeHateLossStrategy with code:{code}, subcode:{subcode}, positions:{positions}, maxPositions:{maxPositions}, debugMode:{debugMode}", level="Info")

        # Constants
        self.stopLossPoint = 50
        self.minEarnPoint = 2
        self.consecutiveCountThreshold = 16
        self.consecutiveAmplitudeThreshold = 3
        self.closeConsecutiveCountThreshold = 8
        self.closeConsecutiveAmplitudeThreshold = 2

        self.kpi = KPI.KPI(api, code, subcode, "InTimeHateLoss", debugMode)
        ...
        while True:
            ...
            if self.kpi.consecutiveUp >= self.consecutiveCountThreshold and \
               self.kpi.consecutiveUpAmplitude >= self.consecutiveAmplitudeThreshold:
                ...
                elif self.positionAction == "S" and  stopLossPrice >= orderPrice >= meanPrice - self.minEarnPoint:
                    # Never lose unless exceeding stopLossPrice!
                    Util.log(f"Attempting to close out positions (buy) but price too high ({orderPrice})", level="Info")
                else:
                    quantity = 1
                    if self.debugMode:
                ...
                elif self.positionAction == "B" and  stopLossPrice <= orderPrice <= meanPrice + self.minEarnPoint:
                    # Never lose unless exceeding stopLossPrice!
                    Util.log(f"Attempting to close out positions (sell) but price too low ({orderPrice})", level="Info")
                else:
                    quantity = 1
                    if self.debugMode:
                        if not self.positions: # No positions
